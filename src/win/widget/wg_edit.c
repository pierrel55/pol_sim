...
static void edit_box_set_num_value(hwin_t *hw, wg_edit_box_t *eb, float n, bool refresh_blit, bool enable_changed_value_cb)
{
  ...
  if (enable_changed_value_cb && eb->num.value_changed_cb && (eb->num.ed_value != n_prev))
    eb->num.value_changed_cb(&eb->wg, eb->num.ed_value);
}
...
static void eb_refresh_blit_text(hwin_t *hw, wg_edit_box_t *eb)
{
  bm_paint_rect(&hw->cli_bm, eb->wg.c_pos.x, eb->wg.c_pos.y, eb->wg.c_size.x, eb->wg.c_size.y, eb->wg.dr_obj.bk_color);
...
{
  wg_edit_box_t *eb = C_NEW(wg_edit_box_t);
  if (eb)
...
void wg_edit_box_num_set_value(hwin_t *hw, widget_t *wg, float n)
{
  ...
  if (n != eb->num.ed_value)
    edit_box_set_num_value(hw, eb, n, true, false);
}
