...
{
  wg_image_t *wg_image = C_NEW(wg_image_t);
  if (wg_image)
  {
...
{
  wg_text_t *wg_text = C_NEW(wg_text_t);
  if (wg_text)
  {
...
void wg_text_update(hwin_t *hw, widget_t *wg, const char *str)
{
  ...
  dx_old = wg->c_size.x;                         // save old size
  text_update(&txt->text, str);
  dx_new = txt->text.size.x;
  if (dx_new > dx_old)                           // do not update now if old blit size > new size
    wg->c_size.x = dx_new;

  wg_draw_blit(hw, wg, true, true, true);        // draw new
  wg->c_size.x = dx_new;                         // update size
}
...
{
  wg_frame_t *wg_frame = C_NEW(wg_frame_t);
  if (wg_frame)
  {
    if (title)
    {
...
// framed text button
static widget_t *get_wg_text_button(const char *name, button_pressed_cb_t pressed_cb)
{
  wg_button_t *bt = C_NEW(wg_button_t);
  if (bt)
  {
...
{
  return win_add_widget(hw, get_wg_text_button(name, pressed_cb));
}
...
{
  wg_combo_t *cb = C_NEW(wg_combo_t);
  if (cb)
  {
    wg_button_t *bt = &cb->text_bt;
    int flags = WG_SHOW|WG_ENFOCUS|WG_SHIFT| ((l_margin < 0) ? WG_CENTER : WG_LMARGIN);
    if (str_count)
      flags |= WG_ENABLE;
    else
...
{
  wg_progress_bar_t *prb = C_NEW(wg_progress_bar_t);
  if (prb)
  {
    prb->wg.c_size.x = 100;
